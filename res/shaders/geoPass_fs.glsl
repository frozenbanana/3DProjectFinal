...
void main() {
  ...
  gNormal = (1.0 - n_tex.z) * normalize(g_nor);
  gNormal += n_tex.z * TBN * (texture(texture_normal0, g_uvs).rgb * 2.0  - 1.0);

  gDiffSpec.rgb = (1.0 - n_tex.x) * vec3(1.0, 1.0, 1.0);
  gDiffSpec.rgb +=  n_tex.x * texture(texture_diffuse0, g_uvs).rgb;
  //gDiffSpec.rgb = texture(texture_normal0, g_uvs).rgb;
  //gDiffSpec.rgb = gNormal;
  //gDiffSpec = vec4(1.0);

  gDiffSpec.a = 0.0;
  gDiffSpec.a += n_tex.y * texture(texture_specular0, g_uvs).r;
